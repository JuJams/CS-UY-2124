/*
hw09.cpp
Linked lists
*/


#include <iostream>
#include <vector>
using namespace std;


class Polynomial{

    // Friend declarations for output stream and equality operators
    friend ostream& operator<<(ostream& os, const Polynomial& rhs);
    friend bool operator==(const Polynomial& rhs, const Polynomial& lhs);
    // Node for linked list
    struct Node{
        Node(int data = 0,Node* next = nullptr):data(data),next(next){}
        int data;
        Node* next;
     };

    public:
        // Default constructor
        Polynomial();

        // Constructor that takes a vector of coeeficients
        Polynomial(const vector<int>& theVector); 

        // Copy Contro : Copy constructor, Destructor, Assignment operator
        // Copy constructor
        Polynomial(const Polynomial& rhs);

        // Destructor
        ~Polynomial();

        // Assignment operator
        Polynomial& operator=(const Polynomial& rhs);

        // += operator
        Polynomial& operator+=(const Polynomial& rhs);

        // To calculate the polynomial's value for a given number
        // Parameters: num
        // Return type: int
        int evaluate(int num) const;

    private:
    // Polynomial member variables
        Node* head;
        size_t degree;
};

// Operator prototypes for + and !=
Polynomial operator+(const Polynomial& rhs, const Polynomial& lhs);
bool operator!=(const Polynomial& rhs, const Polynomial& lhs);

void doNothing(Polynomial pointer) {}
int main() {
        
    //test constructor
    Polynomial p0;                       // 0
    Polynomial p1({17});                 // 17
    Polynomial p2({1, 2});               // x + 2
    Polynomial p3({-1, 5});              // -x + 5
    Polynomial p4({5, 4, 3, 2, 1});      // 5x^4 + 4x^3 + 3x^2 + 2x + 1
    Polynomial has_a_zero({4, 0, 1, 7}); // 4x^3 + x + 7

    // Polynomial pointer(p4);
    // cout << "pointer: " << pointer << endl;
        
    cout << "displaying polynomials\n"
         << "p0: " << p0 << endl
         << "p1: " << p1 << endl
         << "p2: " << p2 << endl
         << "p3: " << p3 << endl
         << "p4: " << p4 << endl
         << "has_a_zero: " << has_a_zero << endl;

    // Polynomial pointer;
    // pointer = p2 + p3;
    // cout << "pointer: " << pointer << endl;
    
    cout << "p2 + p3: " << (p2+p3) << endl
         << "p2 + p4: " << (p2+p4) << endl
         << "p4 + p2: " << (p4+p2) << endl;

    //test copy constructor - the statement below uses the copy constructor
    //to initialize poly3 with the same values as poly4
    Polynomial p5(p4);
    p5 += p3;
    cout << "Polynomial p5(p4);\n"
         << "p5 += p3;\n";

    cout << "p4: " << p4 << endl;  
    cout << "p5: " << p5 << endl;  


    cout << "Calling doNothing(p5)\n";
    doNothing(p5);
    cout << "p5: " << p5 << endl;

    //tests the assignment operator
    Polynomial p6;
    cout << "p6: " << p6 << endl;
    cout << boolalpha;  // Causes bools true and false to be printed that way.
    cout << "(p4 == p6) is " << (p4 == p6) << endl;
    p6 = p4;
    cout << "p6: " << p6 << endl;
    cout << boolalpha;
    cout << "(p4 == p6) is " << (p4 == p6) << endl;

    //test the evaluaton
    int x = 5;
    cout << "Evaluating p1 at " << x << " yields: " << p1.evaluate(5) << endl;
    cout << "Evaluating p2 at " << x << " yields: " << p2.evaluate(5) << endl;
        
    Polynomial p7({3, 2, 1});           // 3x^2 + 2x + 1
    cout << "p7: " << p7 << endl;
    cout << "Evaluating p7 at " << x << " yields: " << p7.evaluate(5) << endl;

    cout << boolalpha;
    cout << "(p1 == p2) is " << (p1 == p2) << endl;
    cout << "(p1 != p2) is " << (p1 != p2) << endl;

    Polynomial p8({ 1, 1 });
    Polynomial p9({ -1, 1 });
    Polynomial p10({ 0, 0, 2 });
    //p8 + p9 tests if += does the cleanup()
    //p10 tests if constructor does the cleanup()

    cout << "p8: " << p8 << endl
         << "p9: " << p9 << endl
         << "p10: " << p10 << endl;

    cout << "((p8 + p9) == p10) is " << ((p8 + p9) == p10) << endl;
}

// Default constructor
        Polynomial::Polynomial() {
            head = new Node();
            degree = 0;
        }

// Constructor that takes a vector of coeeficients
Polynomial::Polynomial(const vector<int>& theVector) {
    if (theVector.size() != 0){
        head = new Node(theVector[0]);
        Node* pointer = head;
        for(size_t i = 1; i < theVector.size(); i++){
            pointer->next = new Node(theVector[i]);
            pointer = pointer->next;
        }
        degree = theVector.size() - 1;
    } 
    else{
        head = new Node();
        degree = 0;
    }
    if(degree == 0) {
        return;
    }
    // Cleaning up leading zeros
    while(head->next && head->data == 0){
        head = head->next;
        --degree;
    }
}

// Copy constructor
Polynomial::Polynomial(const Polynomial& rhs):
        head(new Node(rhs.head->data)) , degree(rhs.degree){
    Node* pointer = head;
    for(const Node* pntr = rhs.head->next; 
        pntr ; pntr = pntr->next){
        pointer->next = new Node(pntr->data);
        pointer = pointer->next;
    }
}

// Destructor
Polynomial::~Polynomial(){
    Node* pointer = head;
    while(pointer){
        Node* pointer2 = pointer;
        pointer = pointer->next;
        delete pointer2;
    }
}

// Assignment operator
Polynomial& Polynomial::operator=(const Polynomial& rhs){
    if(this != &rhs){
        Node* pointer = head;
        while(pointer){
            Node* pointer2 = pointer;
            pointer = pointer->next;
            delete pointer2;
        }
        degree = rhs.degree;
        head = new Node(rhs.head->data);
        Node* pointer2 = head;
        for(const Node* pntr = rhs.head->next; 
        pntr; pntr = pntr->next){
            pointer2->next = new Node(pntr->data);
            pointer2 = pointer2->next;
        }
    }
    return *this;
}

// += operator
Polynomial& Polynomial::operator+=(const Polynomial& rhs) {
    Node* pointer = head;
    const Node* pointer2 = rhs.head;
    size_t len;
    if (degree > rhs.degree) {
        len = degree - rhs.degree;
        while (len > 0) {
            pointer = pointer->next;
            --len;
        }
    }
    else if (degree < rhs.degree) {
        len = rhs.degree - degree;
        for (size_t i = 0; i < len; ++i) {
            head = new Node(0, head);
            pointer = head;
            ++degree;
        }
    }
    for (Node* pntr = pointer; pntr; pntr=pntr->next) {
        pntr->data += pointer2->data;
        pointer2 = pointer2->next;
    }
    if (degree == 0) {
        return *this;
        }
    while (head->next && head->data == 0){
        head = head->next;
        --degree;
    }
    return *this;
}

// To calculate the polynomial's value for a given number
// Parameters: num
// Return type: int
int Polynomial::evaluate(int num) const{
    const Node* pntr = head;
    int result = 0;
    for (size_t i = degree+1;i > 0; --i) {
        size_t powerLen = i-1;
        int power = 1;
        for (size_t j = 0; j < powerLen; ++j){
            power *= num;
        }
        result += power * pntr->data;
        pntr = pntr->next;
    }
    return result;
}

// + operator
Polynomial operator+(const Polynomial& rhs, 
            const Polynomial& lhs) {
    Polynomial result = lhs;
    result += rhs;
    return result;
}

// == operator
bool operator==(const Polynomial& rhs, 
            const Polynomial& lhs){
    if (rhs.degree != lhs.degree){
        return false;}
    const Polynomial::Node* rhsPtr = rhs.head;
    const Polynomial::Node* lhsPtr = lhs.head;
    while(lhsPtr){
        if (lhsPtr->data != rhsPtr->data){
            return false;}
        rhsPtr = rhsPtr->next;
        lhsPtr = lhsPtr->next;
    }
    return true;
}

// != operator
bool operator!=(const Polynomial& rhs, 
            const Polynomial& lhs) {
    return !(lhs == rhs);
}

// output operator
ostream& operator<<(ostream& os, 
        const Polynomial& rhs) {
    const Polynomial::Node* pntr = rhs.head;
    if (rhs.degree != 0) {
        for (size_t i = 0; i < rhs.degree + 1; ++i) {
            if (pntr->data != 0) {
                if ((pntr->data != 1 && pntr->data != -1) || 
                    rhs.degree - i == 0) {
                    os << pntr->data;
                }
                else if(rhs.degree - i != 0 && (pntr->data == 1 || 
                    pntr->data == -1)){
                    if(pntr->data < 0){
                        os<< "-";
                    }
                }
                if (rhs.degree - i > 1) {
                    os << "x^" << rhs.degree - i;
                } 
                else if (rhs.degree - i == 1) {
                    os << 'x';
                }
                if (i != rhs.degree) {
                    os << " + ";
                }
            }
            pntr = pntr->next;
        }
    } 
    else {
        os << rhs.head->data;
    }
    return os;
}
