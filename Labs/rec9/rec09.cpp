...
public:
    PrintedMaterial(unsigned numOfPages):numOfPages(numOfPages){}
...
void PrintedMaterial::displayNumPages() const {
    cout << numOfPages<< endl;     // WHY AM I DOING THIS???? A DUMMY??
}
...
public:
    // TextBook(unsigned numOfPages):PrintedMaterial(numOfPages){} 
    //(Indirect non virtual base class is not allowed)
    TextBook(unsigned numOfPages, unsigned numIndxPgs):Book(numOfPages),
...
// Pure Virtual: Making a method pure virtual makes an abstract class.
// C++ will not allow an object of that type to be created
// Any class deriving from that abstract class must override that pure virtual method
// A concrete class is where all the pureness is overriden
// The real design reason gor pure virtual/abstract is to enforce that all children will have 
// this method and they can define it with their own specifics
