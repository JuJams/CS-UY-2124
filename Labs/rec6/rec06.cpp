...
public:
    ...
    //Constructor
    Directory(const string& someCompany):company(someCompany), size(0),
    capacity(0), entries(new Entry*[0]){}
    ...
    unsigned operator[](const string& someName) const {
        for (size_t i = 0; i < size ; i++){
            if (entries[i]->getName() == someName){
    ...
    // Big 3's (Copy constructor)
    Directory(const Directory& rhs):company(rhs.company),size(rhs.size),
    capacity(rhs.size), entries(new Entry*[rhs.capacity]) {
        for(size_t i = 0; i<size; i++){
